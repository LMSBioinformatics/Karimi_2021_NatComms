...
meta_info$x13 <- 'NOT_NKT'

colnames(meta_info) <- c("rep", "celltype", "Cd4+Cd8+","Cd4+Cd8-", "Cd4-Cd8+", "Cd4-Cd8-", 'Cd8', 'Cd4', 'Itm2a', 'Stat1', "Zbtb7b+Runx3+","Zbtb7b+Runx3-", "Zbtb7b-Runx3+", "Zbtb7b-Runx3-", "NKT_satus") 
...
meta_info[rownames(Rep1_log[(Rep1_log[,'Itgae'] > 0 | Rep1_log[,'Cst7'] > 0 | Rep1_log[,'Nkg7'] > 0),]) ,'NKT_satus'] <- 'NKT'
meta_info[rownames(Rep2_log[(Rep2_log[,'Itgae'] > 0 | Rep2_log[,'Cst7'] > 0 | Rep2_log[,'Nkg7'] > 0),]) ,'NKT_satus'] <- 'NKT'
meta_info[rownames(Rep2KO_log[(Rep2KO_log[,'Itgae'] > 0 | Rep2KO_log[,'Cst7'] > 0 | Rep2KO_log[,'Nkg7'] > 0),]) ,'NKT_satus'] <- 'NKT'
...
draw_Plots <-  function(data_scatter_new, meta_data_f, gene_name){
  ...
  colnames(meta_data_f)[16] <- gene_name
...
P1 <- ggplot(meta_data_f, aes(x = PC1_range, y = meta_data_f[,15]))+geom_jitter(
...
A1 <- table(meta_data_new[meta_data_new$Cd4Cd8=="Cd4+Cd8-" & meta_data_new$PC1_range=="-10_to_-5", 'NKT_satus'])
A2 <- table(meta_data_new[meta_data_new$Cd4Cd8=="Cd4+Cd8-" & meta_data_new$PC1_range=="-5_to_-2.5", 'NKT_satus'])
A3 <- table(meta_data_new[meta_data_new$Cd4Cd8=="Cd4+Cd8-" & meta_data_new$PC1_range=="-2.5_to_0", 'NKT_satus'])
A4 <- table(meta_data_new[meta_data_new$Cd4Cd8=="Cd4+Cd8-" & meta_data_new$PC1_range=="0_to_2.5", 'NKT_satus'])
A5 <- table(meta_data_new[meta_data_new$Cd4Cd8=="Cd4+Cd8-" & meta_data_new$PC1_range=="2.5_to_7.5", 'NKT_satus'])

A <- as.data.frame(c(0,0,0,3,0,34,1,15,1,3))
colnames(A) <- ' cell_counts'
A$PC1_range <- c("-10_to_-5","-10_to_-5","-5_to_-2.5","-5_to_-2.5","-2.5_to_0","-2.5_to_0","0_to_2.5","0_to_2.5","2.5_to_7.5","2.5_to_7.5")
A$PC1_range <- factor(A$PC1_range, levels=c("-10_to_-5","-5_to_-2.5","-2.5_to_0","0_to_2.5","2.5_to_7.5"))
A$NKT_satus <- c('NKT','NOT_NKT','NKT','NOT_NKT','NKT','NOT_NKT','NKT','NOT_NKT','NKT','NOT_NKT')
A$NKT_satus <- factor(A$NKT_satus, levels=c("NOT_NKT","NKT"))

P3 <- ggplot(A, aes(x = PC1_range, y = A[,1])) +
  geom_bar(
    aes(color = NKT_satus, fill = NKT_satus),
    stat = "identity", position = position_dodge(0.8),
    width = 0.7
    )+ theme_classic()+ylab("Number of cells")+
  scale_color_manual(values = c("darkgray", "red"))+
  scale_fill_manual(values = c("darkgray", "red"))+ylim(0,50)+
  scale_color_manual(values = c("darkgray", "red"))+
...
A <- as.data.frame(c(0,0,0,0,1,8,1,9,0,0))
colnames(A) <- ' cell_counts'
A$PC1_range <- c("-10_to_-5","-10_to_-5","-5_to_-2.5","-5_to_-2.5","-2.5_to_0","-2.5_to_0","0_to_2.5","0_to_2.5","2.5_to_7.5","2.5_to_7.5")
A$PC1_range <- factor(A$PC1_range, levels=c("-10_to_-5","-5_to_-2.5","-2.5_to_0","0_to_2.5","2.5_to_7.5"))
A$NKT_satus <- c('NKT','NOT_NKT','NKT','NOT_NKT','NKT','NOT_NKT','NKT','NOT_NKT','NKT','NOT_NKT')
